...
function getAdjacentSquares(x: number, y: number): Array<[number, number]> {
...
function isWallBlocking(state: GameState, fromX: number, fromY: number, toX: number, toY: number): boolean {
...
function getLegalMoves(state: GameState, playerIdx: 0 | 1): Array<{ x: number; y: number; type: string }> {
  const player = state.players[playerIdx]!;
  const opponent = state.players[1 - playerIdx]!;
  const moves: Array<{ x: number; y: number; type: string }> = [];
  
  const adjacent = getAdjacentSquares(player.x, player.y);
  
  for (const [x, y] of adjacent) {
    if (x === opponent.x && y === opponent.y) {
      const jumpMoves = getAdjacentSquares(x, y).filter(([sx, sy]) => 
        state.board[sx]![sy] === 0
      );
...
function hasPathToGoal(state: GameState, playerIdx: 0 | 1): boolean {
  const player = state.players[playerIdx]!;
...
function isLegalWall(state: GameState, wall: Wall): boolean {
  if (wall.x < 1 || wall.x > 8 || wall.y < 1 || wall.y > 8) return false;
  
  for (const existingWall of state.walls) {
    if (existingWall.x === wall.x && existingWall.y === wall.y) return false;
    
    if (wall.orientation === 'h') {
      if (existingWall.orientation === 'h') {
        if (existingWall.y === wall.y && Math.abs(existingWall.x - wall.x) <= 1) return false;
      }
    } else {
      if (existingWall.orientation === 'v') {
        if (existingWall.x === wall.x && Math.abs(existingWall.y - wall.y) <= 1) return false;
      }
    }
  }
  
  const tempState = { ...state, walls: [...state.walls, wall] };
  return hasPathToGoal(tempState, 0) && hasPathToGoal(tempState, 1);
}

function applyMove(state: GameState, playerIdx: 0 | 1, x: number, y: number): GameState {
  const player = state.players[playerIdx]!;
  ...
  newPlayers[playerIdx] = { ...player, x, y };
  
  let winner: 0 | 1 | null = null;
  if (playerIdx === 0 && y === 0) winner = 0;
  if (playerIdx === 1 && y === 8) winner = 1;
  
  return {
    ...state,
    board: newBoard,
    players: newPlayers,
    currentPlayer: winner === null ? (1 - playerIdx) as 0 | 1 : state.currentPlayer,
    winner,
    selectedPlayer: null
  };
}
...
function applyWall(state: GameState, wall: Wall): GameState {
  ...
  newPlayers[state.currentPlayer] = { ...player, walls: player.walls - 1 };
...
function generateBoardHTML(state: GameState): string {
  let html = '';
...
function setupEventHandlers(state: GameState): void {
...
(window as any).toggleWallMode = function() {
